...
from .config import merge_config, async_validate_config
...
class FirstBusConfigFlow(ConfigFlow, domain=DOMAIN): 
  ...
  async def async_step_user(self, user_input):
    ...
    if user_input is not None:
      (errors, config) = await async_validate_config(user_input)
...
class OptionsFlowHandler(OptionsFlow):
  ...
  async def async_step_user(self, user_input):
    ...
    (errors, config) = await async_validate_config(config)
